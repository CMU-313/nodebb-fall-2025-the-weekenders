...
topicsAPI.create = async function (caller, data) {
	...
	try {
		console.error(
			'[DEBUG] api.topics.create - result.postData.user:',
			result && result.postData && result.postData.user
		);
		console.error(
			'[DEBUG] api.topics.create - result.topicData.user:',
			result && result.topicData && result.topicData.user
		);
	} catch (err) {
		console.error(
			'[DEBUG] api.topics.create - logging error',
