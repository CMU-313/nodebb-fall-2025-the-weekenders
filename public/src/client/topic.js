...
define("forum/topic", [
	...
	Topic.init = async function () {
		...
		function onPostEndorsed(data) {
			const $post = $(
				'[component="topic"] [component="post"][data-pid="' + data.pid + '"]',
			);
			if (!$post.length) {
				return;
			}
			...
			$post
				.attr("data-endorsed", "1")
				.attr("data-endorsed-at", data.endorsed_at || Date.now());
			...
			if (!$post.find('[component="post/endorsed"]').length) {
				$post
					...
					.prepend(
						'<span component="post/endorsed" class="badge bg-success me-2 align-middle">Endorsed</span>',
					);
			}
		...
		function updateEndorsedBadgeByEl($post, show) {
			...
			if (!$badge.length) {
				...
				$badge = $(
					'<span component="post/endorsed/badge" class="badge bg-success ms-2">Endorsed</span>',
				);
				($header.length
					? $header
					: $post.find(".post-info, .post-header").first()
				).append($badge);
			}
	...
	function reorderEndorsedPosts() {
		...
		const postList =
			ajaxify && ajaxify.data && Array.isArray(ajaxify.data.posts)
				? ajaxify.data.posts
				: [];
		if (!topicEl.length || !postList.length) {
			return;
		}
